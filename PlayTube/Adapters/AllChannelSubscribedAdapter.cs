...
namespace PlayTube.Adapters
{
                    ...
                    if (Methods.CheckConnectivity())
                    {
                        if (UserDetails.IsLogin)
                        {
                            ...
                            if (e.ButtonSubscribed.Tag.ToString() == "Subscribe")
                            {
                                ...
                                e.ButtonSubscribed.Text = ActivityContext.GetText(Resource.String.Btn_Subscribed);
                                ...
                                PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Global.Add_Subscribe_To_Channel_Http(e.UserClass.Id) });
                            ...
                            else
                            {
                                ...
                                e.ButtonSubscribed.Text = ActivityContext.GetText(Resource.String.Btn_Subscribe);
    ...
    public class AllChannelSubscribedAdapterViewHolder : RecyclerView.ViewHolder
    {
        ...
        public Button BtnSubcribeChannel { get; private set; }
        ...
        public AllChannelSubscribedAdapterViewHolder(View itemView, Action<AllChannelAdapterClickEventArgs> clickListener,Action<AllChannelAdapterClickEventArgs> longClickListener) : base(itemView)
        {
            try
            {
                ...
                BtnSubcribeChannel = MainView.FindViewById<Button>(Resource.Id.SubChannelButton);
                BtnSubcribeChannel.Tag = "Subscribe";
                //Event
                itemView.Click += (sender, e) => clickListener(new AllChannelAdapterClickEventArgs { View = itemView, Position = AdapterPosition });
                itemView.LongClick += (sender, e) => longClickListener(new AllChannelAdapterClickEventArgs { View = itemView, Position = AdapterPosition });
            }
            catch (Exception exception)
            {
                Console.WriteLine(exception);
    ...
    public class AllChannelSubscribedClickEventArgs : EventArgs
    {
        ...
        public Button ButtonSubscribed { get; set; }
    }
}