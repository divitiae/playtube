...
namespace PlayTube.Activities.Channel.Activities
{
    public class ActivitiesViewFragment : Fragment, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, MaterialDialog.IInputCallback
    {
        ...
        private ActivitiesObject ItemActivity;
        private string ActivityId, TypeDialog;
        private CommentObject ItemComments;
                ...
                if (Methods.CheckConnectivity())
                {
                    if (UserDetails.IsLogin)
                    {
                        ...
                        if (!Methods.CheckConnectivity())
                            ...
                            PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.DisLikeActivity_Http(ItemActivity.Id) });
                    }
                    else
                    {
                        PopupDialogController dialog = new PopupDialogController(Activity, null, "Login");
                ...
                if (Methods.CheckConnectivity())
                {
                    if (UserDetails.IsLogin)
                    {
                        ...
                        if (!Methods.CheckConnectivity())
                            ...
                            PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.LikeActivity_Http(ItemActivity.Id) });
                    }
                    else
                    {
                        PopupDialogController dialog = new PopupDialogController(Activity, null, "Login");
        ...
        private async Task LoadDataAsync(string offset = "0")
        {
            ...
            if (Methods.CheckConnectivity())
            {
                ...
                var (apiStatus, respond) = await RequestsAsync.Activities.GetCommentsActivity_Http(ItemActivity.Id, "25", offset);
                if (apiStatus != 200 || !(respond is GetCommentsActivityObject result) || result.Data == null)
                {
                ...
                else
                {
                    ...
                    if (respondList > 0)
                    {
                        ...
                        {
                            MAdapter.CommentList = new ObservableCollection<CommentObject>(result.Data);
                ...
                if (TypeDialog == "EditComment")
                {
                    if (p1 == DialogAction.Positive)
                    {
                        if (Methods.CheckConnectivity())
                        {
                            ...
                            {
                                //Send api delete 
                                PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.EditCommentActivity_Http(ItemActivity.Id, data.Text) });
                            }
                        }
                        else
                        {
                ...
                else if (text == Activity.GetText(Resource.String.Lbl_Delete))
                {
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        if (data != null)
                        {
                            ...
                            //Send api delete 
                            PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.DeleteCommentActivity_Http(ItemActivity.Id) });
                        }
                    }
                    else
                    {
                ...
                if (p1.Length() > 0)
                {
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        if (data != null)
                        {
                            ...
                            //Send api delete 
                            PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.EditCommentActivity_Http(ItemActivity.Id, strName) });
                        }
                    }
                    else
                    {
