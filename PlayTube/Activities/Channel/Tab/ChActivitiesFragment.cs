...
using PlayTube.Activities.Channel.Activities;
    ...
    public class ChActivitiesFragment : Fragment
    {
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            try
            {
                ...
                MAdapter.DisLikeItemClick += MAdapterOnDisLikeItemClick;
                ...
                if (Methods.CheckConnectivity())
                {
                    if (UserDetails.IsLogin)
                    {
                        ...
                        if (!Methods.CheckConnectivity())
                            ...
                            PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.DisLikeActivity_Http(item.Id) });
                    }
                    else
                    {
                ...
                if (Methods.CheckConnectivity())
                {
                    if (UserDetails.IsLogin)
                    {
                        ...
                        if (!Methods.CheckConnectivity())
                            ...
                            PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Activities.LikeActivity_Http(item.Id) });
                    }
                    else
                    {
                        PopupDialogController dialog = new PopupDialogController(Activity, null, "Login");
        ...
        private async Task LoadDataAsync(string offset = "0")
        {
            ...
            if (Methods.CheckConnectivity())
            {
                ...
                var (apiStatus, respond) = await RequestsAsync.Activities.GetActivities_Http(IdChannel, "25", offset);
                if (apiStatus != 200 || !(respond is GetActivitiesObject result) || result.Data == null)
                {
                    MainScrollEvent.IsLoading = false; 
                ...
                else
                {
                    ...
                    { 
                        ...
                        {
                            MAdapter.ActivitiesList = new ObservableCollection<ActivitiesObject>(result.Data);
